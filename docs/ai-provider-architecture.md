...
>   - 仍未迁移：`QwenDocumentParseService`（若仍存在且未被主链路使用，列为后续清理）、Writing guidance workflow（`generateWritingGuidance` / `extractHeadingsWithAi` 仍走旧 DeepSeek HttpClient，列为 Phase 5C-3 或 cleanup）。
